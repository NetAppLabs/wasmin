...
function parseGithubUrl(
  githubUrl: string,
  secretStore?: any
): { gitHubConfig: GithubConfig; newUrl: string } {
  ...
  const gConfig = new GithubConfig(
    owner,
    repo,
    ref,
    githubUser,
...
export class GithubFolderHandle
  implements ImplFolderHandle<GithubFileHandle, GithubFolderHandle>
{
  constructor(
    config: GithubConfig,
    path: string,
  ...
  public async queryPermission(
    _options: FileSystemHandlePermissionDescriptor
    ...
    try {
      ...
      for (const item of data.tree || []) {
        ...
        if (itemType == "blob") {
          ...
          //isDir
          this._entries[entryName] = new GithubFolderHandle(
            this.config,
            path,
            entryName,
            writeable
  ...
  ): Promise<GithubFileHandle | undefined> {
    const entry = this._entries[name];
...
export class GithubRepoHandle extends GithubFolderHandle {
  constructor(githubUrl: string, secretStore?: any) {
    const { gitHubConfig: config, newUrl: newUrl } = parseGithubUrl(
      githubUrl,
      secretStore
    );
...
export class GithubRepoListHandle
  implements ImplFolderHandle<GithubFileHandle, GithubRepoHandle>
{
  constructor(githubUrl: string, secretStore?: any) {
    const { gitHubConfig: config, newUrl: newUrl } = parseGithubUrl(
      githubUrl,
  ...
  async populateEntries() {
    try {
      ...
      for (const item of data || []) {
        ...
        this._entries[itemName] = new GithubRepoHandle(
          repoGithubUrl,
          this.secretStore
        );
      }
...
function GetRepoHandle(
  githubUrl: string,
  secretStore?: any
): GithubRepoHandle | GithubRepoListHandle {
  const { gitHubConfig: config, newUrl: newUrl } = parseGithubUrl(
    githubUrl,
    secretStore
  );
...
export default (githubUrl: string, secretStore?: any) =>
  GetRepoHandle(githubUrl, secretStore);
