...
function rimraf(
  evt: any,
  toDelete: FileHandle | FolderHandle,
  recursive = true
) {
  ...
  implements ImplFolderHandle<FileHandle, FolderHandle>
{
  ...
  async *entries(): AsyncGenerator<[string, FileHandle | FolderHandle]> {
    ...
    for (const [name, [id, isFile, isExternal]] of Object.entries(entries)) {
      if (isFile) {
        ...
        const extHandle = await this.getExternalFolderHandle(
          name,
          id,
          isFile,
          isExternal
      ...
      req.onsuccess = () => {
        ...
        entry // entry exist
          ? entry[1] // isFile?
            ...
            : resolve(
                this.getExternalFolderHandle(name, entry[0], entry[1], entry[2])
              )
          : do_create
  ...
  async getExternalFolderHandle(
    ...
    if (cachedEntry) {
      ...
      if (isFile) {
        ...
        const handeRetrieved = new Promise<FileSystemDirectoryHandle>(
          (resolve, reject) => {
  ...
  async verifyPermission(
    ...
    if (fileHandle.requestPermission) {
      ...
      console.warn(
        "fileHandle.requestPermission does not exist on ",
        fileHandle
      );
    }
  ...
  async removeEntry(
    ...
    return new Promise<void>((resolve, reject) => {
      ...
      cwdQ.onsuccess = (evt) => {
        ...
        const toDeleteFileOrFolder = toDeleteFileOrFolderUnknown as
          | FileHandle
          | FolderHandle;
        if (!toDelete._) {
  ...
  async openFile(subPath: string): Promise<File> {
    ...
    const fh = await openFileHandle(
      new NFileSystemDirectoryHandle(thisAsFHandle),
      subPath
    );
