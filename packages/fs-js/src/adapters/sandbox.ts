        ...
        return new Promise((resolve, reject) =>
            this.file.createWriter((fileWriter) => {
                if (opts.keepExistingData === false) {
                    fileWriter.onwriteend = (_evt) => resolve(new Sink(fileWriter, this.file));
...
export class FolderHandle {
    ...
    async *entries() {
        ...
        for (const x of entries) {
            yield [
                x.name,
                x.isFile ? new FileHandle(x as any, this.writable) : new FolderHandle(x as any, this.writable),
            ];
        }
    }
    ...
    async removeEntry(name: string, opts: { recursive: any }) {
        const entry: Error | FolderHandle | FileHandle = await this.getDirectoryHandle(name).catch((err) =>
