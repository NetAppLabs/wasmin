 ...
 * */
export function substituteSecretValue(
  secretKey: string,
  secretStore: any
): string {
  ...
  for (let i = 0; i < pathLength; i++) {
    ...
    if (pathPart != "") {
      ...
      if (is_last) {
        if (expectingFile) {
          utilDebug(
            `openHandle: expectingFile: ${expectingFile} before getFileHandle pathPart: ${pathPart}`
          );
        ...
        utilDebug(
          `openHandle: before getDirectoryHandle pathPart: ${pathPart}`
        );
