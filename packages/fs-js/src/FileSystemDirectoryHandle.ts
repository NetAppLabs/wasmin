  ...
  implements FileSystemDirectoryHandle
{
    ...
    for await (const [name, _] of this.adapter.entries()) {
      if (name.endsWith(NFileSystemDirectoryHandle.LINK_SUFFIX)) {
        const newName = name.replace(
          NFileSystemDirectoryHandle.LINK_SUFFIX,
          ""
        );
        yield newName;
      } else {
  ...
  async getFileHandle(
    ...
    if (name === "." || name === ".." || name.includes("/"))
      throw new TypeError(`Name contains invalid characters.`);
    try {
      const f = new NFileSystemFileHandle(
  ...
  async resolve(
    possibleDescendant: NFileSystemHandle
  ): Promise<string[] | null> {
