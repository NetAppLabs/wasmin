...
export class S3File implements File {
  ...
  async arrayBuffer(): Promise<ArrayBuffer> {
    ...
    s3Debug(
      `S3File: arrayBuffer: start: ${start}" end: ${end} contentType: ${contentType}`
    );
...
function parseS3Url(
  s3Url: string,
  secretStore?: any
): { s3config: S3Config; newUrl: string } {
  let newUrl = s3Url;
...
export class S3FolderHandle
  implements ImplFolderHandle<S3FileHandle, S3FolderHandle>
{
        ...
        if (contents != undefined) {
          for (const cont of contents) {
            ...
            if (entryName != "") {
              ...
              s3Debug(
                `populateEntries lastModifiedMillis: ${lastModifiedMillis}`
              );
              ...
              this._entries[entryName] = new S3FileHandle(
                this.config,
                path,
                entryName,
                file,
        ...
        if (prefixes != undefined) {
          for (const pref of prefixes) {
            ...
            if (spref != undefined) {
              ...
              this._entries[entryName] = new S3FolderHandle(
                this.config,
                path,
                entryName,
                writeable
  ...
  ): Promise<S3FileHandle | undefined> {
    const entry = this._entries[name];
...
export class S3BucketListHandle
  implements ImplFolderHandle<S3FileHandle, S3BucketHandle>
{
  constructor(s3Url: string, secretStore?: any) {
    const { s3config: config, newUrl: newUrl } = parseS3Url(s3Url, secretStore);
    const name = "s3";
      ...
      if (contents != undefined) {
        for (const cont of contents) {
          ...
          if (entryName) {
            ...
            this._entries[entryName] = new S3BucketHandle(
              s3BucketUrl,
              this.secretStore
            );
          }
...
function GetBucketHandle(
  s3Url: string,
  secretStore?: any
): S3BucketHandle | S3BucketListHandle {
  const { s3config: config, newUrl: newUrl } = parseS3Url(s3Url, secretStore);
...
export default (s3Url: string, secretStore?: any) =>
  GetBucketHandle(s3Url, secretStore);
