...
export class MetaLoader implements WasmLoader {
    async instantiate(relPath: string): Promise<Uint8Array> {
        debugMessage("MetaLoader: relPath: ", relPath);
        const wasmUrl = "." + relPath + ".wasm";
        const metaUrl = new URL(wasmUrl, import.meta.url);
