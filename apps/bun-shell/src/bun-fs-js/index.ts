  ...
  async write(chunk: any) {
    ...
    if (chunk instanceof ArrayBuffer) {
      ...
      for await (const data of chunk.stream()) {
        ...
        const written = fsSync.writevSync(this.fileHandle,
          [data as Buffer],
          this.position
        );
...
export class BunFolderHandle
  implements ImplFolderHandle<BunFileHandle, BunFolderHandle>
{
  ...
  resolve(
    _possibleDescendant: BunFileHandle | BunFolderHandle
  ): Promise<string[] | null> {
  ...
  public async *entries(): AsyncGenerator<
    readonly [string, BunFileHandle | BunFolderHandle],
    void,
    unknown
  > {
    const dir = this.path;
